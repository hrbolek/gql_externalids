...
def createPageTest(tableName, queryEndpoint, attributeNames=["id", "name"], subEntities=[]):
    # attlist = ' '.join(attributeNames)
    @pytest.mark.asyncio
    async def result_test():
        async_session_maker = await prepare_in_memory_sqllite()
        ...
        # query = "query{" f"{queryEndpoint}" "{" + attlist + "}}"
        query = createPageQuery(queryEndpoint=queryEndpoint, attributeNames=(attributeNames + subEntities))
        print("query", query)
...
def createResolveReferenceTest(tableName, gqltype, attributeNames=["id", "name"], subEntities=[]):
    @pytest.mark.asyncio
    async def result_test():
        async_session_maker = await prepare_in_memory_sqllite()
        await prepare_demodata(async_session_maker)
